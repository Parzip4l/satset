...
@section('title', 'Data Master Impact')
@section('title-sub', 'Master Data')
@section('pagetitle', 'Impact List')
        ...
        <div class="col-12">
            ...
            <div class="card mb-4 border-0 shadow-sm" style="border-radius: 12px;">
                <div class="card-body p-4">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                        <div>
                            <h4 class="fw-bold mb-1">Data Master Impact</h4>
                            <p class="text-muted mb-0">Kelola tingkat dampak masalah terhadap bisnis.</p>
                        </div>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-primary shadow-sm px-4" data-bs-toggle="modal" data-bs-target="#ModalDivisi">
                                <i class="bi bi-plus-lg me-1"></i> Tambah Impact
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="card border-0 mb-4 shadow-sm" style="border-radius: 12px;">
                <div class="card-body p-3">
                    <form id="filter-form" method="GET" action="{{ route('impact.index') }}">
                        <div class="row g-3 align-items-center">
                            ...
                            <div class="col-md-6">
                                <div class="position-relative">
                                    <input type="text" id="search-input" name="search" class="form-control ps-5" 
                                           ...
                                           placeholder="Cari nama impact atau kode..."> 
                            ...
                            <div class="col-md-2 ms-auto">
                                <a href="{{ route('impact.index') }}" class="btn btn-outline-danger w-100">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                                ...
                                @forelse($impact as $data)
                                    ...
                                    <tr>
                                        <td class="ps-4 py-3 text-muted fw-semibold">
                                            {{ ($impact->currentPage() - 1) * $impact->perPage() + $loop->iteration }}
                                        </td>
                                        <td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-5">
                                            ...
                                            <h6 class="text-muted fw-normal">Data impact tidak ditemukan.</h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <div class="card-footer bg-white border-top py-3">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                        <div class="text-muted small fw-medium">
                            Menampilkan {{ $impact->firstItem() ?? 0 }} sampai {{ $impact->lastItem() ?? 0 }} dari {{ $impact->total() }} data
                        </div>
                        <div class="pagination-container">
                            {{ $impact->appends(request()->query())->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg">
            <div class="modal-header border-bottom-0 pb-0">
                <h5 class="modal-title fw-bold">Tambah Impact Baru</h5>
            ...
            <div class="modal-body p-4">
                <form action="{{ route('impact.store')}}" method="POST">
                    ...
                    <div class="mb-3">
                        <label class="form-label small text-uppercase fw-bold text-muted">Nama Impact</label>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="code" class="form-control" placeholder="Contoh: IMP-01" required>
                    </div>
...
<div class="modal fade" id="ModalDivisiUpdate{{$data->id}}" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg">
            <div class="modal-header border-bottom-0 pb-0">
                <h5 class="modal-title fw-bold">Edit Impact</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <form action="{{ route('impact.update', $data->id)}}" method="POST">
                    @csrf
                    @method('PUT')
...
<script>
    ...
    function confirmDelete(id, name) {
        Swal.fire({
            title: 'Hapus Impact?',
            text: "Impact '" + name + "' akan dihapus permanen!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            Swal.fire('Terhapus!', 'Data impact berhasil dihapus.', 'success')
